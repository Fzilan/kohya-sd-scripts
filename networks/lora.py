...
class LoRAModule(nn.Cell):
    ...
    def __init__(
        ...
        if isinstance(org_module, nn.Conv2d):
            ...
            self.lora_down = nn.Linear(in_dim, self.lora_dim, has_bias=False)
            self.lora_up = nn.Linear(self.lora_dim, out_dim, has_bias=False)
        else:
            NotImplementedError
    ...
    def save_weights(self, file, dtype, metadata=None):
        ...
        for n, p in self.unet.parameters_and_names():
            if ".lora_" in n or ".alpha" in n:
                ckpt_lora.append(
                    {"name": self.LORA_PREFIX_UNET + "." + n, 
                     "data": p if dtype is None else p.to(dtype=dtype)}
                )
