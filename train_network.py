    ...
    def train(self, args):
        ...
        for epoch in range(num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader_iter):
                ...
                logger.info(f"current step loss: {loss.numpy().item()}, current step time cost: {time.time() - s:.2f} s")
...
class TrainStepForSDXLLoRA(TrainStep):
    def __init__(
        ...
        self.max_timestep = noise_scheduler.config.num_train_steps if args.max_timestep is None else args.max_timestep
