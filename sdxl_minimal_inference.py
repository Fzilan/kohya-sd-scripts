...
if __name__ == "__main__":
    ...
    def generate_image(prompt, prompt2, negative_prompt, seed=None):
        ...
        # prepare embedding
        # vector
        emb1 = get_timestep_embedding(ms.FloatTensor([original_height, original_width]).unsqueeze(0), 256)
        emb2 = get_timestep_embedding(ms.FloatTensor([crop_top, crop_left]).unsqueeze(0), 256)
        emb3 = get_timestep_embedding(ms.FloatTensor([target_height, target_width]).unsqueeze(0), 256)
        # logger.info("emb1", emb1.shape)
        c_vector = ops.cat([emb1, emb2, emb3], axis=1).to(dtype=DTYPE)
        uc_vector = c_vector.copy().to(
            dtype=DTYPE
